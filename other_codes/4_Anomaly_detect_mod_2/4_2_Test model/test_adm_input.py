...
#REPORT PARAMETERS
id = '2.7.1' #test ID that will be printed in the report name header
...
threshold_out = 0.15
model_name = f'adm_2_7_b  (No threshold. follows anomalies. value limits 1/0 applied. Normalization applied.)' #(WITH THRESHOLD OUTPUT CLASSIFICATION {threshold_out}) same as adm_13 but trained not to follow anomalies'
tf_model_name = 'adm_2_7_b'  #The existing model

model_type = 'MLP'
model_desc = '83x32x32x32x6 MLP to detect anomalies on 3 panels based on the previous 20 points from 3 panels + 20 time points + derivatives of 3 panels. Tested on previously unseen data. Trained to detect sensor spikes, const regions, temp shifts and bumps (removed duplicates)'# and costant regions, temperature shifts and bumps. '
model_size = 'NA'
n_points_inp = 83
...
date_time = '22.06.2023'#formatted_date_time #Change this if you want the time to be something else