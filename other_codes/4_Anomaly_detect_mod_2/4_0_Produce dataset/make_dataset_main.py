...
def telemetry_files_to_dataset(N, filenames, out_file_name, directory, normal_min, normal_max):
    ...
    with open(out_file_name, "w") as output_file:
        ...
        values1 = telemetry_to_str_list(filename_1)
        values2 = telemetry_to_str_list(filename_2)
        values3 = telemetry_to_str_list(filename_3)
...
def make_telemetry_files_with_anomalies():
    ...
    for t_part in t_parts:
        ...
        if t_a_spike:
            ...
            for i in range(7):
                ind += 1
                fil_lst = telemetry_to_str_list(file1)
        ...
        if t_a_bump:
            ...
            if dir == 1:
                ...
                a = -1
            fil_lst = bump_temp(fil_lst, a, random.randint(10, len(list)-17), random.randint(9, 17), random.randint(80, 120))
            shift_file = list_to_telemetry_file(fil_lst, file1[:-4] + f'_a_{ind}.txt', Telemetry_dir)
            ind += 1
            continue
...
def produce_dataset():
    ...
    with open('training_dataset_1_3.txt', 'a') as output_file:
        ...
        for filename in os.listdir(Dataset_dir):
            if (not '1187' in filename) and (filename[-1] == '0') and (
                    filename[-2] == '_'):  # SELECT THE PART TO BE LEFT FOR TESTING, in this case 1187
...
if test_anomaly_functions:
    for telemetry_file_name in os.listdir(Telemetry_dir):
        if main_panel in telemetry_file_name and (telemetry_file_name == 'output_+X_44.txt'):
            for i in range(6):
